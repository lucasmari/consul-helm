...
func (h *HelmCluster) checkForPriorInstallations(t *testing.T) {
	...
	// Wait for all pods labelled with app=consul to exit. A previous
	...
	retry.RunWith(&retry.Counter{Wait: 1 * time.Second, Count: 30}, t, func(r *retry.R) {
		consulPods, err := h.kubernetesClient.CoreV1().Pods(h.helmOptions.KubectlOptions.Namespace).List(context.Background(), metav1.ListOptions{LabelSelector: "app=consul"})
		require.NoError(r, err)
